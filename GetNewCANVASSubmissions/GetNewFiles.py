...
class DirectoryDiff:
    def __init__(self, master):
        ...
        self.get_differences_button = tk.Button(master, text="Get file differences", command=self.GetNewFiles)
...
#        NewDirectoryNameOnly = os.path.split(self.NewDirectoryName)[1]
    ...
    def GetNewFiles(self):
        ...
        os.chdir(self.NewDirectoryName)
        os.chdir("..")
        DirectoryForNewFiles= os.path.split(self.NewDirectoryName)[1] + "_new_only"
        os.mkdir(DirectoryForNewFiles)
        os.chdir(DirectoryForNewFiles)

        for fn in NewFiles:
            print("copying: ",self.NewDirectoryName + "/" + fn)
            copy(self.NewDirectoryName + "/" + fn,".")

        LogFilename = "DirectoryDifferences.log"
...
root = tk.Tk()
DirectoryDiff(root)
root.title("Compare files and put new files in a new directory")
root.mainloop()